...
from core import helpers, ipdb

# Create a linux brigde, with e.g 'homework' as name
bridge_name='homework'
ipdb_h = ipdb.Ipdb()
ipdb_h.connect()
ipdb_h.create_interface_bridge(bridge_name)
ipdb_h.change_status_interface(bridge_name, 'up')
...
for (ip_last_digit, netnspath) in zip(ips_last_digit[0:len(netnspaths)], netnspaths_ids):
    ipdb_ns = ipdb.Ipdb()
    ipdb_ns.connect(netnspath)
    veth, veth_peer = ipdb_h.create_interface('veth0', ip_last_digit)
    ipdb_h.move_interface_tonetns(veth, netnspath)
    ipdb_h.add_port_interface(bridge_name,veth_peer)
    ipdb_h.change_status_interface(veth_peer, 'up')
    ipdb_ns.change_status_interface(veth, 'up')
    ipdb_ns.change_addr_interface(veth, net+ip_last_digit, mask)
